...
/** Fire-and-forget, debounced snapshot of a room's full state, called after
 * every broadcast (see socketHandlers.ts) so a mid-deploy restart can bring
 * every live room back exactly as it was instead of wiping it out from
 * under whoever's mid-round. Debounced per room code so rapid clicking (a
 * host tapping +/- on strokes repeatedly) doesn't hammer the DB with a
 * write per click — a real crash/redeploy still has far more than
...
export function saveRoomSnapshot(room: Room): void {
  ...
  const timer = setTimeout(() => {
    ...
    const snapshot = normalizeForSnapshot(room);
    db.execute({
      ...
      args: [code, JSON.stringify(snapshot)],
    }).catch((err) => {
 ...
 * repopulates the in-memory room Map (see roomStore.ts's
 * restoreRoomsFromSnapshots) from whatever was live the moment before the
...
export async function loadRoomSnapshots(): Promise<Room[]> {
  ...
  return res.rows.map((row) => JSON.parse((row as unknown as { data: string }).data) as Room);
}
