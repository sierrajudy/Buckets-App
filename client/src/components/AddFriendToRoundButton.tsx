 ...
 * than in place — both the Lobby and Scorecard render this button inside a
 * card with `backdrop-blur-sm`, and `backdrop-filter` on an ancestor makes
 * it the containing block for any `position: fixed` descendant (same as
 * `transform`/`filter`/`will-change`). Without the portal, the "fixed"
 * panel was actually positioned relative to that blurred card instead of
 * the viewport — landing nowhere near the button and rendering hazy, since
 * it was compositing inside the same blurred layer. */
