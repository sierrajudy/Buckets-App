...
export function CourseSearch({
  ...
  // Once a course+tee is already set, collapse to a "Change course" button
  // rather than leaving the search box sitting there — editing reopens it.
  ...
  async function handlePickTee(teeKey: string) {
    ...
    try {
      ...
      onSelect(course);
      resetSearch();
      setEditing(false); // collapse back to the "Change course" button
    } catch (err) {
    ...
    return (
      <div className="mt-3">
        <button
          ...
          className="text-xs text-neutral-500 hover:text-green-600 dark:hover:text-green-400"
        >
          Change course
        </button>
      </div>
